...
interface EditImagingFormProps {
  trigger: React.ReactElement<{ onClick?: () => void }>;
  record: ImagingIndicator;
  onOk?: () => void;
}

const EditImagingForm: React.FC<EditImagingFormProps> = ({
  trigger,
  record,
  onOk,
}) => {
  ...
  const { run, loading } = useRequest(
    (values: Parameters<typeof updateImagingIndicator>[1]) =>
      updateImagingIndicator(record.id, values),
    {
      manual: true,
      onSuccess: () => {
...
export default EditImagingForm;
